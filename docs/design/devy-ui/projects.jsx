  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      <div style={{ padding: "24px 36px 16px", borderBottom: "1px solid var(--hairline-soft)", display: "flex", alignItems: "center", gap: 14 }}>
        <h1 className="t-display-xl" style={{ margin: 0, letterSpacing: -0.6 }}>Projects</h1>
        <div style={{ width: 1, height: 22, background: "var(--hairline)", margin: "0 6px" }} />
        {/* project switcher — pill row when ≤4, dropdown otherwise */}
        <ProjectSwitcher
...
// ---------- Project switcher (pill row ≤4, dropdown ≥5) ----------
const ProjectSwitcher = ({ projects, activeId, setActiveId, onNew }) => {
  const active = projects.find((p) => p.id === activeId);
  const useDropdown = projects.length > 4;
    ...
    return (
      <div style={{ display: "flex", gap: 4, alignItems: "center", flexWrap: "wrap" }}>
        ...
        <button key={p.id} onClick={() => setActiveId(p.id)} style={{
          ...
          background: p.id === activeId ? "var(--surface-strong)" : "transparent",
          color: p.id === activeId ? "var(--ink)" : "var(--muted)",
          fontSize: 13, fontWeight: p.id === activeId ? 600 : 500
        }}>
            <span style={{ width: 8, height: 8, borderRadius: 999, background: p.color }} />
            {p.name}
            <span className="t-mono muted" style={{ fontSize: 10, marginLeft: 4 }}>{p.cards.length}</span>
          </button>
        )}
        <Button variant="ghost" size="sm" icon="plus" onClick={onNew}>New project</Button>
      </div>);
  ...
  return (
    <div style={{ position: "relative" }}>
      <button onClick={() => setOpen((o) => !o)} style={{
        ...
        fontSize: 13, fontWeight: 600, minWidth: 220, textAlign: "left"
      }}>
        <span style={{ width: 8, height: 8, borderRadius: 999, background: active.color }} />
        <span style={{ flex: 1 }}>{active.name}</span>
        <span className="t-mono muted" style={{ fontSize: 10 }}>{active.cards.length}</span>
      ...
      <>
          ...
          borderRadius: 10, boxShadow: "0 12px 36px rgba(0,0,0,.18)", overflow: "hidden"
        }}>
            <div style={{ padding: "8px 10px", borderBottom: "1px solid var(--hairline-soft)" }}>
              <input autoFocus value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search projects…"
            style={{ width: "100%", padding: "6px 10px", borderRadius: 6, border: "1px solid var(--hairline-soft)",
              fontSize: 12, outline: "none", background: "var(--canvas)", color: "var(--ink)" }} />
            </div>
            <div style={{ overflowY: "auto", flex: 1, padding: 4 }}>
              {filtered.map((p) =>
            <button key={p.id} onClick={() => {setActiveId(p.id);setOpen(false);setQ("");}} style={{
            ...
            onMouseEnter={(e) => {if (p.id !== activeId) e.currentTarget.style.background = "var(--surface-soft)";}}
            onMouseLeave={(e) => {if (p.id !== activeId) e.currentTarget.style.background = "transparent";}}>
                  ...
                  <span className="t-mono muted" style={{ fontSize: 10 }}>{p.cards.length}</span>
                </button>
            )}
              {filtered.length === 0 && <div style={{ padding: 16, textAlign: "center", color: "var(--muted-soft)", fontSize: 12 }}>No matches</div>}
            </div>
            <button onClick={() => {setOpen(false);onNew();}} style={{
            borderTop: "1px solid var(--hairline-soft)",
            padding: "10px 14px", border: "none", background: "transparent",
            cursor: "pointer", textAlign: "left", fontSize: 12, color: "var(--primary)", fontWeight: 600
          }}>+ New project</button>
          </div>
        </>
      }
