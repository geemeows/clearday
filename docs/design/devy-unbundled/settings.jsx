...
const QuietHoursCard = () => {
  ...
  return (
    <div className="card" style={{ padding: 18 }}>
      ...
      <div className="t-body-sm muted" style={{ marginBottom: 8, fontSize: 12, lineHeight: 1.5 }}>
        Signals matching any of these <b style={{ color: "var(--ink)" }}>bypass rules</b> override quiet hours and ring through. For richer event-driven logic (post to Slack on PR ready, reply during Focus, etc.) use{" "}
        <a onClick={() => window.dispatchEvent(new CustomEvent("devy:nav", { detail: { tab: "automations" } }))} style={{ color: "var(--primary)", cursor: "pointer", textDecoration: "underline", textDecorationStyle: "dotted", textUnderlineOffset: 2 }}>Automations</a>
        {" "}instead.
      </div>
      <BypassRulesEditor />
    </div>);
...
// ---- Bypass-rules editor: picks from a curated palette + free entry ----
const BYPASS_PRESETS = [
"@mentions", "CI red on prod", "On-call pages",
"Slack DMs from manager", "PRs marked Urgent", "Calendar starts in <10m",
"Linear · P0", "GitHub · review-requested:@me"];

const BypassRulesEditor = () => {
  const [rules, setRules] = useS_s(["@mentions", "CI red on prod", "On-call pages"]);
  const [open, setOpen] = useS_s(false);
  const [draft, setDraft] = useS_s("");
  const remove = (t) => setRules((r) => r.filter((x) => x !== t));
  const add = (t) => {const v = (t || "").trim();if (!v || rules.includes(v)) return;setRules((r) => [...r, v]);};
  ...
  return (
    <div ref={ref} style={{ position: "relative" }}>
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
        {rules.map((t) =>
        <span key={t} className="chip" style={{ background: "var(--primary-disabled)", color: "var(--primary-active)", fontSize: 12, display: "inline-flex", alignItems: "center", gap: 4 }}>
            {t}
            <button onClick={() => remove(t)} style={{ border: "none", background: "transparent", color: "inherit", cursor: "pointer", padding: 0, marginLeft: 2, fontSize: 13, lineHeight: 1, opacity: 0.7 }}>×</button>
          </span>
        )}
        <button onClick={() => setOpen((o) => !o)} className="chip chip-outline"
        style={{ border: "1px dashed var(--hairline)", cursor: "pointer", fontSize: 12, color: "var(--muted)" }}>+ Add rule</button>
      </div>
      {open &&
      <div style={{
        position: "absolute", top: "calc(100% + 6px)", left: 0, zIndex: 30,
        width: 360, background: "var(--popover)", border: "1px solid var(--border)",
        borderRadius: "var(--radius-md)", boxShadow: "var(--shadow-card)", padding: 8
      }}>
          <input autoFocus value={draft} onChange={(e) => setDraft(e.target.value)}
        placeholder="Type a rule, or pick one below…"
        onKeyDown={(e) => {if (e.key === "Enter") {add(draft);setDraft("");setOpen(false);}}}
        style={{
          width: "100%", padding: "7px 10px", borderRadius: "var(--radius-sm)",
          border: "1px solid var(--input)", background: "var(--background)",
          fontSize: 13, color: "var(--foreground)", outline: "none", marginBottom: 8
        }} />
          <div className="t-tag" style={{ fontSize: 9.5, padding: "4px 4px" }}>Common picks</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 1, maxHeight: 200, overflowY: "auto" }}>
            {BYPASS_PRESETS.filter((p) => !rules.includes(p) && (!draft || p.toLowerCase().includes(draft.toLowerCase()))).map((p) =>
          <button key={p} onClick={() => {add(p);setDraft("");setOpen(false);}}
          style={{
            display: "flex", alignItems: "center", gap: 8, width: "100%", padding: "6px 8px",
            border: "none", background: "transparent", borderRadius: 6, cursor: "pointer",
            fontSize: 12.5, color: "var(--foreground)", textAlign: "left"
          }}
