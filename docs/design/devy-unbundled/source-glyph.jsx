// Source glyphs — brand marks for the providers Devy connects to.
// Uses simplified, recognisable SVG marks (GitHub, Slack, Google Calendar,
// Linear, Jira). Drawn as monochrome marks coloured per-source via tokens.

const BRAND_PATHS = {
  // GitHub Octocat silhouette (single-path simplified, simple-icons style)
  git: {
    viewBox: "0 0 24 24",
    paths: [
  ...
  // Google Calendar — colorful square w/ "31" date
  cal: {
    viewBox: "0 0 24 24",
    paths: [
  ...
  // Linear — simple-icons path: 3 stacked diagonal segments + a corner triangle
  linear: {
    viewBox: "0 0 24 24",
    paths: [
  ...
  // Generic task fallback (kept for legacy 'task' key, points to Linear)
  task: {
    viewBox: "0 0 24 24",
    paths: [
...
const SourceGlyph = ({ source, size = 22 }) => {
  // AI keeps its sparkle character — it's not a third-party brand
  if (source === "ai") {
    return (
      <div style={{
        width: size, height: size,
        background: "var(--src-ai-bg)", color: "var(--src-ai)",
        borderRadius: 6,
  ...
  const cfg = {
    ...
    jira:   { bg: "var(--src-jira-bg, #DEEBFF)", fg: "var(--src-jira, #2684FF)", brand: "jira" },
  }[source];
  ...
  if (!cfg) {
    return (
      <div style={{
        ...
        borderRadius: 6,
        display: "inline-flex", alignItems: "center", justifyContent: "center",
        flexShrink: 0, fontSize: size * 0.55, fontWeight: 700,
      }}>?</div>
    );
  }
...
const SourceLabel = ({ source }) => {
  const labels = {
    git: "GitHub", slack: "Slack", cal: "Calendar",
    task: "Linear", linear: "Linear", jira: "Jira", ai: "AI",
  };
