...
// Pure compute — per-criterion + per-competency from per-indicator scores & targets.
function computeSatisfaction(level) {
  ...
  for (const c of level.competencies) {
    ...
    for (const cr of c.criteria) {
      ...
      const crTar = inds.length ? inds.reduce((s,i)=>s+(i.target ?? cr.target ?? 3),0)/inds.length : (cr.target ?? 3);
      out.perCriterion[cr.id] = { avg: crAvg, target: crTar, gap: crTar - crAvg };
      scores = scores.concat(inds.map(i=>i.score));
      targets = targets.concat(inds.map(i => i.target ?? cr.target ?? 3));
    }
    const current = scores.length ? scores.reduce((a,b)=>a+b,0) / scores.length : 0;
    const target = targets.length ? targets.reduce((a,b)=>a+b,0) / targets.length : 0;
    out.perCompetency.push({ id: c.id, name: c.name, current, target, gap: target - current });
  }
  return out;
}

// Migrate seed data: each indicator gets its own target (copied from parent criterion target),
// and 0-scores are bumped to 1 (no zero state in the new scale).
for (const c of ACTIVE_LEVEL.competencies) {
  for (const cr of c.criteria) {
    for (const ind of cr.indicators) {
      if (typeof ind.target !== "number") ind.target = cr.target ?? 3;
      if (ind.score === 0) ind.score = 1;
    }
  }
}

window.CareerData = { CAREER_LEGEND, CAREER_LEGEND_DESC, ACTIVE_LEVEL, ARCHIVED_LEVELS, computeSatisfaction };

// Development plan — initial seed
ACTIVE_LEVEL.development_plan = [
  { id: "dp1", title: "Lead RFC for v2 ingest pipeline", start: "May 1, 2026",  due: "Jun 14, 2026", status: "in_progress", criterion_id: "cr_sys" },
  { id: "dp2", title: "Mentor Sam through their first design doc",   start: "May 6, 2026", due: "Jun 30, 2026", status: "in_progress", criterion_id: "cr_mentor" },
  { id: "dp3", title: "Drive cross-team rollout of Devy → Career", start: "May 12, 2026", due: "Jul 8, 2026", status: "not_started", criterion_id: "cr_cross" },
];
